...
export async function rebuildLeaderboard() {
  ...
  const teamTotals = rows.reduce<Map<string, number>>((totals, row) => {
    if (!row._id.teamId) {
      return totals;
    }
  ...
  await Team.updateMany({}, { totalPoints: 0 });
  await Promise.all(
    [...teamTotals.entries()].map(([teamId, totalPoints]) =>
      Team.findByIdAndUpdate(teamId, { totalPoints }),
    ),
  );
